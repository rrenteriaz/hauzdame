...
export default function CreateVariantGroupModal({
  ...
  useEffect(() => {
    if (focusLastOptionRef.current && optionsContainerRef.current) {
      ...
      const inputs = optionsContainerRef.current.querySelectorAll<HTMLInputElement>('input[placeholder^="Variante"]');
    ...
    >
      ...
      >
        <h3 className="text-base font-semibold text-neutral-900 mb-1">
          Crear grupo de variantes — {itemName}
        </h3>
        {isBedSizeVariantable && (
          <p className="text-xs text-neutral-500 mb-3">
            Este ítem suele usar el grupo &quot;Tamaño de cama&quot; con opciones como Individual, Matrimonial, Queen y King.
          </p>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-neutral-700 mb-1">
              Nombre del grupo (key) *
            </label>
            <input
              type="text"
              value={key}
              onChange={(e) => setKey(e.target.value)}
              placeholder="Ej: tipo_copas, bed_size"
