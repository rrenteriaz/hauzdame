// app/host/actividad/limpiezas/proximas/page.tsx
import { getDefaultTenant } from "@/lib/tenant";
import { getAllUpcomingCleanings } from "@/app/host/hoy/data";
import Page from "@/lib/ui/Page";
import ListContainer from "@/lib/ui/ListContainer";
import ListRow from "@/lib/ui/ListRow";
import { safeReturnTo } from "@/lib/navigation/safeReturnTo";

export default async function LimpiezasProximasPage({
  searchParams,
}: {
  ...
  const items = await getAllUpcomingCleanings(tenant.id, propertyId);
  ...
  const currentViewUrl = `/host/actividad/limpiezas/proximas${currentViewParams.toString() ? `?${currentViewParams.toString()}` : ""}`;

  return (
    <Page title="Próximas limpiezas" showBack backHref={returnTo}>
      {items.length === 0 ? (
