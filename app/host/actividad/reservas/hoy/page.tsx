// app/host/actividad/reservas/hoy/page.tsx
import { getDefaultTenant } from "@/lib/tenant";
import { getAllTodayReservations } from "@/app/host/hoy/data";
import Page from "@/lib/ui/Page";
import ListContainer from "@/lib/ui/ListContainer";
import ListRow from "@/lib/ui/ListRow";
import { safeReturnTo } from "@/lib/navigation/safeReturnTo";

export default async function ReservasHoyPage({
  searchParams,
}: {
  ...
  const items = await getAllTodayReservations(tenant.id, propertyId);
  ...
  const currentViewUrl = `/host/actividad/reservas/hoy${currentViewParams.toString() ? `?${currentViewParams.toString()}` : ""}`;

  return (
    <Page title="Reservas para hoy" showBack backHref={returnTo}>
      {items.length === 0 ? (
