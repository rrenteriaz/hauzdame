// app/host/actividad/reservas/proximas/page.tsx
import { getDefaultTenant } from "@/lib/tenant";
import { getAllUpcomingReservations } from "@/app/host/hoy/data";
import Page from "@/lib/ui/Page";
import ListContainer from "@/lib/ui/ListContainer";
import ListRow from "@/lib/ui/ListRow";
import { safeReturnTo } from "@/lib/navigation/safeReturnTo";

export default async function ReservasProximasPage({
  searchParams,
}: {
  ...
  const items = await getAllUpcomingReservations(tenant.id, propertyId);
  ...
  const currentViewUrl = `/host/actividad/reservas/proximas${currentViewParams.toString() ? `?${currentViewParams.toString()}` : ""}`;

  return (
    <Page title="Próximas reservas" showBack backHref={returnTo}>
      {items.length === 0 ? (
        ...
        <ListContainer>
          {items.map((item, index) => {
            ...
            const detailHref = `/host/reservations/${item.id}?${detailParams.toString()}`;
            ...
            return (
              <ListRow
                ...
                href={detailHref}
                isLast={isLast}
                ariaLabel={`Ver detalles de reserva ${item.propertyName}`}
              >
